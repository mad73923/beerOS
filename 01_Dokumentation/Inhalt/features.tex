...
Die Bitfelder \texttt{prev} und \textt{next} geben den Index des vorherigen bzw. nächsten MemoryHead im Heap Array an.
Durch die Limitierung auf 12 Bit können maximal $2^12$ Segmente bzw $4*2^12 = 16kb$ addresiert werden.
...
Da für \texttt{size} 8 Bit bereitstehen können Speicherbereiche mit einer Größe von bis zu $4*2^8 = 1kb$ allokiert werden.

Es existiert zu jeder Zeit mindestens eine MemoryHead, und zwar das erste Segment des Heaps.
...
Mit der Funktion \texttt{memoryManagement_next} können iterativ alle freien Speicherbereiche erfragt werden, die eine angegebene Größe nicht unterschreiten.
Die erwünschte Größe wird als Teil der Struktur \textt{MemoryRequest} übergeben.
...
Um einen Speicherbereich zu belegen steht die Funktion \texttt{memoryManagement_alloc} zur Verfügung.
Ihr wird beim Aufrufen ebenfalls ein \textt{MemoryRequest} übergeben.
Die in der \textt{MemoryRequest} Stuktur enthaltene Id gibt dabei an welcher Speicherbereich belegt werden soll.
