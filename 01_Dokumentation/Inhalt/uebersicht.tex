...
die erste Task ausgeführt und der Dispatcher 
(s. \ref{sect:dispatcher}) erstmalig durchlaufen.
Dabei wird auch der Scheduler aufgerufen, der, abhängig vom
...
gewählt. Jedoch wurde während der Umsetzung darauf geachtet, 
...
Dieser löst, unabhängig vom Ausführungszustand\footnote{Es sei
denn, die Task befindet sich in einem kritischen 
(Interrupt gesperrten) Abschnitt} der aktuellen Task, einen 
Interrupt aus, in dessen \gls{ISR} der Dispatcher ausgeführt
...
wird es, wenn die Ausführung des interrupt-geschützten Bereichs länger als 
$t_{per}$ dauert,
...
mittels Canary-Wert (s. \ref{sect:stack}) den Stack auf einen Überlauf. Sollte dies
...
Ist zum gleichen Zeitpunkt keine der Tasks im lauffähigen 
...
Außerdem steht eine Iteratorfunktion zur Verfügung, die in Kombination mit einer \texttt{while}-Schleife eine Art \texttt{forEach}-Schleife ergibt.
